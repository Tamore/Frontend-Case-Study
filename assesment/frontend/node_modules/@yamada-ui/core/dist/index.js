...
__export(src_exports, {
  ...
  atRuleProperties: () => atRuleProperties,
  atRuleSelectors: () => atRuleSelectors,
  ...
  getCreateThemeVars: () => getCreateThemeVars,
  getMinMaxQuery: () => getMinMaxQuery,
  ...
  keyframes: () => keyframes,
  layoutStyleProperties: () => layoutStyleProperties,
  memo: () => memo2,
  mergeMultiStyle: () => mergeMultiStyle,
  ...
  pickStyle: () => pickStyle,
  processSkipProperties: () => processSkipProperties,
  ...
  useComponentMultiStyle: () => useComponentMultiStyle,
  useComponentStyle: () => useComponentStyle,
  useCreateVars: () => useCreateVars,
...
var import_sheet = require("@emotion/sheet");
...
function isCSSFunction(value) {
  return (0, import_utils.isString)(value) && value.includes("(") && value.includes(")");
}
...
function splitValues(values, cb = (current) => current === ",", addSeparator = false) {
  ...
  for (let i = 0; i < values.length; i++) {
    ...
    if (current === "(") depth++;
    if (current === ")") depth--;
    if (!(0, import_utils.isUndefined)(current) && cb(current, prev, next) && depth === 0) {
...
function tokenToVar(token, value) {
  return function(theme) {
    var _a;
    const match = (0, import_utils.isString)(value) ? value.match(/fallback\(([^,)]+),?\s*([^]+)?\)/) : null;
    const [, resolvedValue, fallbackValue] = match != null ? match : [];
    if (resolvedValue) value = resolvedValue;
    const resolvedToken = `${token}.${value}`;
    if ((0, import_utils.isObject)(theme.__cssMap) && resolvedToken in theme.__cssMap) {
...
function keyframes(...arg) {
  return (0, import_react.keyframes)(...arg);
}
...
// src/config/animation.ts
var styleSheet = (0, import_utils2.createdDom)() ? new import_sheet.StyleSheet({ key: "css", container: document.head }) : void 0;
...
function animation(value, theme, css2, _prev) {
  ...
  if ((0, import_utils2.isObject)(value)) {
    const {
    ...
    const { name, styles: styles2 } = (0, import_react2.keyframes)(keyframes2);
...
var generateAtRule = (identifier) => (values, theme) => {
  if (!(0, import_utils4.isArray)(values)) return values;
  return values.reduce(
    (prev, {
      ...
      if (!query) {
        const resolvedRest = (0, import_utils4.filterUndefined)({
          height,
          maxHeight,
          maxWidth,
          minHeight,
        ...
        query = Object.entries(resolvedRest).map(([key, value]) => {
          ...
          return `(${(0, import_utils4.toKebabCase)(key)}: ${value})`;
        }).join(" and ");
      }
...
function getValue(value, fallbackValue = "") {
  return function(token, theme, ...rest) {
    ...
    if (prevent) {
      ...
      if ((0, import_utils6.isNumeric)(value)) return value;
      const resolvedToken = `${token}.${value}`;
...
function gradient(value, theme, _css, _prev) {
  ...
  if (!(0, import_utils9.isUndefined)(direction)) colors.unshift(direction);
  const computedValues = colors.map((_color) => {
    var _a, _b;
    if ((0, import_utils9.isUndefined)(_color)) return _color;
    ...
    if (ratio) {
      return [color, ...(0, import_utils9.isArray)(ratio) ? ratio : [ratio]].join(" ");
    } else {
      return color;
    }
  });
...
function generateStyles(prefix) {
  return function(value, theme, _css, prev = {}) {
    const resolvedCSS = {};
    const style = (0, import_utils12.getMemoizedObject)(
      theme,
...
function insertObject(obj, segments, value) {
  ...
  if (!(0, import_utils14.isUndefined)(head)) obj[head] = insertObject((_a = obj[head]) != null ? _a : {}, rest, value);
  return obj;
}
function valueToToken(token, value) {
  return function(theme) {
    const flattedObj = (0, import_utils14.flattenObject)(value);
    return Object.entries(flattedObj).reduce((prev, [path, value2]) => {
      const segments = path.split(".");
      if ((0, import_utils14.isArray)(value2)) {
  ...
  return function(theme) {
    ...
    if ((0, import_utils14.isObject)(value)) {
      value = Object.entries(value).reduce((prev, [key, value2]) => {
        if ((0, import_utils14.isObject)(value2)) {
          prev[key] = valueToToken(token, value2)(theme);
        } else if ((0, import_utils14.isArray)(value2)) {
          prev[key] = value2.map((value3) => {
            if ((0, import_utils14.isObject)(value3)) {
...
function vars(values, theme, _css, _prev) {
  if (!(0, import_utils14.isArray)(values)) return values;
...
var transforms = {
  ...
  content: (value) => {
    if ((0, import_utils16.isObject)(value)) {
  ...
  deg: (value) => {
    ...
    return isUnitless || (0, import_utils16.isNumber)(value) ? `${value}deg` : value;
  },
  filter: generateFilter,
  fraction: (value) => {
    if ((0, import_utils16.isNumber)(value) && value <= 1) value = `${value * 100}%`;
    return value;
  },
  ...
  px: (value) => {
    ...
    return isUnitless || (0, import_utils16.isNumber)(value) ? `${value}px` : value;
  },
...
var import_utils27 = require("@yamada-ui/utils");
...
function transformBreakpoints(breakpoints, options) {
  return Object.fromEntries(
    Object.entries(breakpoints).map(([name, value]) => [name, (0, import_utils19.getPx)(value)]).sort((a, b) => {
...
function expandColorModeArray(key, value, queries2) {
  let computedCSS = {};
  if ((0, import_utils20.isObject)(value[0])) {
...
function expandResponsiveObject(key, value, queries2) {
  return queries2.reduce((prev, { breakpoint, query }) => {
    ...
    } else if ((0, import_utils20.isArray)(breakpointValue)) {
      prev = (0, import_utils20.merge)(prev, expandColorModeArray(key, breakpointValue, queries2));
    } else {
...
function expandAdditionalObject(key, value, queries2) {
  return Object.entries(value).reduce((prev, [query, value2]) => {
    var _a, _b;
    if (query === "base") {
      if ((0, import_utils20.isArray)(value2)) {
        prev = (0, import_utils20.merge)(prev, expandColorModeArray(key, value2, queries2));
      } else {
        prev[key] = value2;
      }
    } else {
      query = (_b = (_a = queries2.find(({ breakpoint }) => breakpoint === query)) == null ? void 0 : _a.query) != null ? _b : query;
      if ((0, import_utils20.isObject)(value2)) {
        prev = (0, import_utils20.merge)(prev, {
          [query]: expandResponsiveObject(key, value2, queries2)
        });
      } else if ((0, import_utils20.isArray)(value2)) {
        prev = (0, import_utils20.merge)(prev, {
...
function expandCSS(css2) {
  return function(theme) {
    ...
    for (let [key, value] of Object.entries(css2)) {
      value = (0, import_utils20.runIfFunc)(value, theme);
      if (value == null) continue;
      if ((0, import_utils20.isArray)(value) && !isProcessSkip(key)) {
        computedCSS = (0, import_utils20.merge)(
          computedCSS,
          expandColorModeArray(key, value, queries2)
        );
        continue;
      }
      if ((0, import_utils20.isObject)(value) && isResponsive(value) && !isProcessSkip(key)) {
        computedCSS = (0, import_utils20.merge)(
          computedCSS,
      ...
      if ((0, import_utils20.isObject)(value) && isAdditionalObject(value)(keys) && !isProcessSkip(key)) {
        computedCSS = (0, import_utils20.merge)(
          computedCSS,
      ...
      computedCSS = (0, import_utils20.merge)(computedCSS, { [key]: value });
    }
    return computedCSS;
  };
}
function valueToVar(value, theme) {
  var _a, _b, _c;
  if ((0, import_utils20.isArray)(value) || (0, import_utils20.isObject)(value)) {
    return value;
  } else if ((0, import_utils20.isString)(value)) {
    ...
    return value.replace(/\$([^,)/\s]+)/g, (_, value2) => {
      var _a2;
      if ((0, import_utils20.isObject)(theme.__cssMap) && value2 in theme.__cssMap) {
...
function css(cssOrFunc) {
  return function(theme, disableStyleProp) {
    function createCSS(cssOrFunc2, isNested = false) {
      var _a, _b, _c, _d, _e;
      const cssObj = (0, import_utils20.runIfFunc)(cssOrFunc2, theme);
      ...
      for (let [prop, value] of Object.entries(computedCSS)) {
        ...
        if ((0, import_utils20.isObject)(value) && !(style == null ? void 0 : style.isProcessSkip)) {
          ...
          resolvedCSS[prop] = (0, import_utils20.merge)(resolvedCSS[prop], createCSS(value, true));
          continue;
        }
        ...
        if (!isNested && (style == null ? void 0 : style.static)) {
          const staticStyles = (0, import_utils20.runIfFunc)(style.static, theme);
          resolvedCSS = (0, import_utils20.merge)(resolvedCSS, staticStyles);
        }
        const properties = (0, import_utils20.runIfFunc)(style == null ? void 0 : style.properties, theme);
        if (properties) {
          if ((0, import_utils20.isArray)(properties)) {
            for (const property of properties) {
              resolvedCSS[property] = value;
            }
            continue;
          } else if ((0, import_utils20.isObject)(value)) {
            resolvedCSS = (0, import_utils20.merge)(resolvedCSS, value);
            continue;
          } else {
...
var useCSS = (cssObject = {}) => {
  const theme = (0, import_react4.useContext)(import_react3.ThemeContext);
  return (0, import_react4.useMemo)(() => getCSS(cssObject)(theme), [cssObject, theme]);
};
var getCSS = (cssObject = {}) => (theme) => (0, import_css.css)(css(cssObject)(theme));
...
var useCreateVars = (obj, keys, options) => {
  const theme = (0, import_react6.useContext)(import_react5.ThemeContext);
...
var createVars = (obj, keys, { format = defaultFormat, transform: transform2 = false } = {}) => (theme) => {
  ...
  const prefix = (_c = (_b = (_a = theme.__config) == null ? void 0 : _a.var) == null ? void 0 : _b.prefix) != null ? _c : DEFAULT_VAR_PREFIX;
  const flattedObj = (0, import_utils21.flattenObject)(obj, {
    separator: "$$",
  ...
  Object.entries(flattedObj).forEach(([path, value], index) => {
    ...
    if (map.has(name)) {
      ...
      const additionalValue = rest.reduceRight(
        (acc, key) => ({ [key]: acc }),
        (0, import_utils21.isObject)(value) ? value : { base: value }
      );
      value = (0, import_utils21.merge)(variable.value, additionalValue);
      map.set(name, { ...variable, value });
    } else {
      const formattedName = format(name, index);
      const style = styles[name];
      const token = (0, import_utils21.isObject)(style) ? style.token : void 0;
      const additionalValue = rest.reduceRight(
        (acc, key) => ({ [key]: acc }),
        (0, import_utils21.isObject)(value) ? value : { base: value }
      );
...
function getCreateThemeVars(prefix = DEFAULT_VAR_PREFIX, queries2) {
  function tokenToVar2(token) {
    ...
    const variable = `--${[prefix, (0, import_utils22.escape)(token, "-")].filter(Boolean).join("-")}`;
  ...
  return function(tokens) {
    return function({
      ...
      function valueToVar2(value) {
        if ((0, import_utils22.isString)(value)) {
          return value.replace(/\$([^,)/\s]+)/g, (_, value2) => {
            var _a, _b;
      ...
      function createNegativeVar(token, reference) {
        ...
        const negativeReference = import_utils22.calc.negate(reference);
        return { negativeReference, negativeToken };
      }
      function createAnimationVar(value) {
        if ((0, import_utils22.isArray)(value)) {
      ...
      function createVar(token, value, variable) {
        return function(isSemantic, queries3 = []) {
          if ((0, import_utils22.isArray)(value)) {
          ...
          } else if ((0, import_utils22.isObject)(value)) {
            ...
            cssVars = (0, import_utils22.merge)(
              cssVars,
...
var EnvironmentContext = (0, import_react7.createContext)(defaultEnvironment);
var EnvironmentProvider = ({
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime2.jsxs)(EnvironmentContext.Provider, { value: context, children: [
    children,
    enabled ? /* @__PURE__ */ (0, import_jsx_runtime2.jsx)("span", { id: "__ui_dev", ref, hidden: true }) : null
  ] });
};
EnvironmentProvider.displayName = "EnvironmentProvider";
var useEnvironment = ({ isDefer } = {}) => {
  const [, forceUpdate] = (0, import_react7.useReducer)((c) => c + 1, 0);
  (0, import_utils23.useSafeLayoutEffect)(() => {
    if (!isDefer) return;
    forceUpdate();
  }, [isDefer]);
  return (0, import_react7.useContext)(EnvironmentContext);
};
...
var ColorModeProvider = ({
  ...
  const [colorMode, setColorMode] = (0, import_react8.useState)(
  ...
  const { getSystemColorMode, setClassName, setDataset } = (0, import_react8.useMemo)(
    () => getColorModeUtils({
      environment,
  ...
  const changeColorMode = (0, import_react8.useCallback)(
    (colorMode2) => {
  ...
  const value = (0, import_react8.useMemo)(
    () => ({
      changeColorMode: defaultColorMode ? import_utils24.noop : changeColorMode,
      ...
      toggleColorMode: defaultColorMode ? import_utils24.noop : toggleColorMode
    }),
    [
      defaultColorMode,
      resolvedColorMode,
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime3.jsx)(ColorModeContext.Provider, { value, children });
};
var useColorMode = () => {
  const context = (0, import_react8.useContext)(ColorModeContext);
  if ((0, import_utils24.isEmptyObject)(context)) {
    return {
      changeColorMode: import_utils24.noop,
      colorMode: "light",
      forced: false,
      internalColorMode: "light",
      toggleColorMode: import_utils24.noop
    };
  } else {
    return context;
  }
...
var useSystemColorMode = ({
  ...
  const callbackRef = (0, import_utils24.useCallbackRef)(callback);
  const [colorMode, setColorMode] = (0, import_react8.useState)(
    initialColorMode
  );
  const { getSystemColorMode, systemColorModeObserver } = (0, import_react8.useMemo)(
    () => getColorModeUtils({ environment }),
    [environment]
  );
  (0, import_utils24.useSafeLayoutEffect)(() => {
    setColorMode(getSystemColorMode());
  }, [getSystemColorMode]);
  (0, import_react8.useEffect)(() => {
    return systemColorModeObserver((0, import_utils24.funcAll)(setColorMode, callbackRef));
...
function getCreateThemeTokens(breakpoints, responsive) {
  return function(theme, target = "primary") {
    ...
    const defaultTokenMap = (0, import_utils25.pickObject)(theme, defaultTokens);
    const semanticTokenMap = (0, import_utils25.pickObject)((_a = theme.semantics) != null ? _a : {}, semanticTokens);
    const defaultTokenEntries = Object.entries(
      (0, import_utils25.flattenObject)(defaultTokenMap, { omitKeys, shouldProcess })
    ).map(([token, value]) => {
      const enhancedToken = { isSemantic: false, value };
      return [token, enhancedToken];
    });
    const semanticTokenEntries = Object.entries(
      (0, import_utils25.flattenObject)(semanticTokenMap, { omitKeys, shouldProcess })
      ...
      if (token.startsWith("colorSchemes.")) {
        ...
        if (tone) {
          ...
          import_utils25.TONES.forEach((tone2) => {
            const enhancedToken = {
              isSemantic: true,
              value: (0, import_utils25.isArray)(value) ? [`${value[0]}.${tone2}`, `${value[1]}.${tone2}`] : `${value}.${tone2}`
            };
    ...
    return (0, import_utils25.objectFromEntries)([
      ...defaultTokenEntries,
...
function omitTheme(theme) {
  return (0, import_utils25.omitObject)(theme, ["__cssMap", "__cssVar", "__breakpoints"]);
}
function omitThemeProps(props, keys = []) {
  return (0, import_utils25.omitObject)(props, ["size", "variant", "colorScheme", ...keys]);
}
...
function recursiveMergeStyle(target, source) {
  ...
  if ((0, import_utils25.isObject)(source) && (0, import_utils25.isObject)(target)) {
    for (const [sourceKey, sourceValue] of Object.entries(source)) {
      const targetValue = target[sourceKey];
      if (target.hasOwnProperty(sourceKey)) {
        if (!(0, import_utils25.isFunction)(targetValue) && !(0, import_utils25.isFunction)(sourceValue)) {
          ...
          result[sourceKey] = (props) => recursiveMergeStyle(
            (0, import_utils25.runIfFunc)(targetValue, props),
            (0, import_utils25.runIfFunc)(sourceValue, props)
          );
        }
      } else {
...
function filterStyle(target) {
  return function({ isMulti = false, omit, pick }) {
    if (!(0, import_utils25.isObject)(target)) return target;
    if (omit.length)
      target = internalFilterStyle(target, omit, isMulti)(import_utils25.omitObject);
    if (pick.length)
      target = internalFilterStyle(target, pick, isMulti)(import_utils25.pickObject);
    return target;
  };
}
function internalFilterStyle(target, keys, isMulti, refs = []) {
  return function(func) {
    ...
    Object.entries(result).forEach(([nestedKey, style]) => {
      ...
      if ((0, import_utils25.isFunction)(style)) {
        result[nestedKey] = (props) => internalFilterStyle(style(props), newKeys, isMulti, newRefs)(func);
      } else {
        if (func === import_utils25.omitObject || Object.keys(style).some((key) => newKeys.includes(key))) {
          ...
          result[nestedKey] = (0, import_utils25.merge)(
            result[nestedKey],
...
function pickStyle(target, targetKey, withProps = true) {
  ...
  Object.entries(target).forEach(([key, value]) => {
    switch (key) {
      case "baseStyle":
        if ((0, import_utils25.isFunction)(value)) {
...
var ThemeProvider = ({
  ...
  const environment = useEnvironment();
  const [themeScheme, setThemeScheme] = (0, import_react10.useState)(
    themeSchemeManager2.get(config == null ? void 0 : config.initialThemeScheme)(storageKey)
  );
  const changeThemeScheme = (0, import_react10.useCallback)(
    (themeScheme2) => {
  ...
  const theme = (0, import_react10.useMemo)(
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime5.jsxs)(import_react9.ThemeProvider, { theme: { changeThemeScheme, themeScheme, ...theme }, children: [
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(
    import_react9.Global,
    {
      styles: (theme) => {
        ...
        const style = (0, import_utils26.getMemoizedObject)(theme, "styles.resetStyle", {});
        const computedStyle = (0, import_utils26.runIfFunc)(style, {
          colorMode,
          theme,
          themeScheme
        });
        if ((0, import_utils26.isEmptyObject)(computedStyle)) return void 0;
...
var GlobalStyle = () => {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime5.jsx)(
    import_react9.Global,
    {
      styles: (theme) => {
        const { themeScheme } = theme;
        let style = (0, import_utils26.getMemoizedObject)(theme, "styles.globalStyle", {});
        const computedStyle = (0, import_utils26.runIfFunc)(style, {
          colorMode,
          theme,
          themeScheme
...
var useTheme = () => {
  const internalTheme = (0, import_react10.useContext)(import_react9.ThemeContext);
  const theme = (0, import_react10.useMemo)(() => {
    var _a;
    const { themeScheme } = internalTheme;
    if ((0, import_utils26.isUndefined)(themeScheme) || themeScheme === "base") return internalTheme;
    const nestedTheme = (_a = internalTheme.themeSchemes) == null ? void 0 : _a[themeScheme];
    if (!nestedTheme) return internalTheme;
    return (0, import_utils26.merge)(internalTheme, nestedTheme);
  }, [internalTheme]);
...
function getColorModeStyles(value, modifierStyles, props) {
  return function({ isMulti = false, selectors = [] }) {
    ...
    return (0, import_utils27.merge)(lightStyles, darkStyles);
  };
}
...
function getResponsiveStyles(value, modifierStyles, props) {
  return function({ isMulti = false, selectors = [] }) {
    var _a, _b, _c;
    const breakpoints = (0, import_utils27.keysFormObject)(value);
      ...
      return queries2.reduce(
        (prev, { breakpoint, maxW, maxWQuery, minW, minWQuery }, index) => {
          ...
          if (!hasBaseStyles) {
            ...
            prev = (0, import_utils27.merge)(prev, styles3);
            hasBaseStyles = true;
          }
          ...
          prev = (0, import_utils27.merge)(prev, styles2);
          return prev;
        },
        {}
      );
...
function getModifierStyles(value, modifierStyles, props) {
  return function({
    ...
    if ((0, import_utils27.isArray)(value)) {
      styles2 = getColorModeStyles(
        value,
        modifierStyles,
        props
      )({ isMulti, selectors });
    } else if ((0, import_utils27.isObject)(value)) {
...
function getStyles(stylesOrFunc, props) {
  return function({
    ...
    const styles2 = (0, import_utils27.runIfFunc)(stylesOrFunc, props);
    if (isMulti) {
      return Object.fromEntries(
        Object.entries(styles2 != null ? styles2 : {}).map(
          ([name, styleOrFunc]) => {
            const style = (0, import_utils27.runIfFunc)(styleOrFunc, props);
            if (selectors.length) {
...
function mergeProps(props, defaultProps, overrideProps) {
  if (defaultProps) props = (0, import_utils27.merge)(defaultProps, props);
  if (overrideProps) props = (0, import_utils27.runIfFunc)(overrideProps, props);
  return props;
}
function useStyles(name, props, { isMulti, isProcessSkip: isProcessSkip2, styles: styles2 = {} } = {}) {
  const { theme, themeScheme } = useTheme();
  const { colorMode } = useColorMode();
  const propsRef = (0, import_react11.useRef)({});
  const stylesRef = (0, import_react11.useRef)(isProcessSkip2 ? styles2 : {});
  if (!isProcessSkip2) {
    const componentStyle = (0, import_utils27.getMemoizedObject)(
      theme,
      `components.${name}`
    );
    props = (0, import_utils27.filterUndefined)(props);
    if (componentStyle) {
      ...
      const colorScheme = (0, import_utils27.isArray)(_colorScheme) ? getColorModeValue(_colorScheme[0], _colorScheme[1])(colorMode) : _colorScheme;
      const computedProps = { ...props, colorScheme };
      const resolvedProps = (0, import_utils27.omitObject)(computedProps, ["children"]);
      let styles3 = {};
      if (baseStyle) {
      ...
      if (sizes) {
        ...
        styles3 = (0, import_utils27.merge)(styles3, sizeStyles);
      }
      if (variants) {
        ...
        styles3 = (0, import_utils27.merge)(styles3, variantStyles);
      }
  ...
  const isPropsEqual = (0, import_react_fast_compare.default)(propsRef.current, props);
...
var import_utils28 = require("@yamada-ui/utils");
var import_react12 = require("react");
var emotionStyled = (0, import_utils28.interopDefault)(import_styled.default);
...
function toCSSObject({
  ...
  return function(props) {
    ...
    const propsCSS = (0, import_utils28.filterObject)(
      rest,
      (prop) => prop in styleProps
    );
    const baseCSS = (0, import_utils28.runIfFunc)(baseStyle, props);
    const computedCSS = css(
      (0, import_utils28.assignAfter)({}, __css, baseCSS, (0, import_utils28.filterUndefined)(propsCSS), sx)
...
function styled(el, { baseStyle, disableStyleProp, ...styledOptions } = {}) {
  ...
  const UIComponent = (0, import_react12.forwardRef)((props, ref) => {
    const { colorMode, forced } = useColorMode();
    return (0, import_react12.createElement)(Component, {
      ref,
